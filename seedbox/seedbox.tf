...
resource "aws_instance" "seedbox" {
  ...
  tags                   = merge(map("Name", "Chiaseedbox${count.index + 1}-${var.instance_name_tag}", "application", var.application_tag, "ref", var.ref,), var.extra_tags)
